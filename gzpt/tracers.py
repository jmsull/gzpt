...
class AutoCorrelator(hzpt):
    ...
    def Xi(self):#,R_excl=None,sigma_excl=0.09):
        '''Returns callable that incorporates the current parameters'''
        if(self.useExc):
            if(self.nmax==1):
    ...
    def wp_grad():
        ...
        gxi = self.Xi()(r,wantGrad=True)[1]
        dpi = pi_bins[1]-pi_bins[0]
        gwp = np.zeros(int(len(xi)/len(pi_bins)))
        #sum over los direction in each bin
        for i in range(len(gwp)-1):
            gwp[i] = 2* dpi * np.sum(gxi[i*len(pi_bins):(i+1)*len(pi_bins)])
        rp = r[::len(pi_bins)]
        return rp,gwp
...
class CrossCorrelator(hzpt):
    ...
    def wp(self,r,pi_bins=np.linspace(0,100,10,endpoint=False)):
        ...
        wp, projected correlation function
        """
        #As in corrfunc code of Sinha & Garrison
        xi = self.Xi()(r)
        dpi = pi_bins[1]-pi_bins[0]
        wp = np.zeros(int(len(xi)/len(pi_bins)))
        #sum over los direction in each bin
