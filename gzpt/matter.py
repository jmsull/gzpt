...
class Correlator(hzpt):
    ...
    1.cosmology object (gives zeldovich)
    2.nmax value to get the starting guess for broadband'''
    def __init__(self,hzpt,nmax):
        ...
        if(self.nmax==0):
            ...
            self.params = np.array([A0_init,R_init,R1h_init, R1sq_init,R2h_init]) #A0, R, R1h, R1, R2h I think
            self.pnames = names
        else:
    ...
    def Power(self,params=[]):
        ...
        '''FIXME: z-dependence of BB parameters  '''
    ...
    def Xi(self,params=[]):
        ...
        if(len(params)<1):params=self.params
        def xi(r):
            #return r**2 * (self.Xi_zel(r,self.z) + bb.XiBB(r,params,nmax=self.nmax))
